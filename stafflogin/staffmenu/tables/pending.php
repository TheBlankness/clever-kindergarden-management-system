 					...
 					<table>
 						<thead>
 							<tr class="table100-head">
 								...
 								<th style="padding-left: 40px;">Plan</th>
...
while($row=mysqli_fetch_assoc($qry) and $row2=mysqli_fetch_assoc($qry2) )//repeat for each record
  {
	...
	//	if($registered=="Pending"){
			...
			echo "<td >$phone</td>";
			echo "	<td >$email</td>";
			echo '	<td style="padding-left: 10px;">'.$age.'</td>';
			echo "	<td style='padding-left: 40px;'>$plans</td>";
			if($registered == "Pending" ){
			echo "	<td style='padding-left: 40px;'><a href='approve.php?approve=yes&parentID=$parentID&email=$email&fullname=$fullname1' style='color: #00FF00;'>Approve</a> / <a href='approve.php?approve=no&parentID=$parentID&email=$email&fullname=$fullname1' style='color: #FF0000;'>Decline</a></td>";
			}else {
